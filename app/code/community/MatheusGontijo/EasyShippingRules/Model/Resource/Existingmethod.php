 ...
 * @license     OSL v3.0
 */
class MatheusGontijo_EasyShippingRules_Model_Resource_Existingmethod
    extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    {
        $this->_init('easyshippingrules/existing_method', 'easyshippingrules_existing_method_id');
    }
    ...
    /**
     * Assign existing shipping rule to store views and linking rules
     *
     * @param Mage_Core_Model_Abstract $object
     *
     * @return Mage_Core_Model_Resource_Db_Abstract
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        if ($object->hasExistingMethodRules()) {
            $rules = Mage::helper('adminhtml/js')
                ->decodeGridSerializedInput($object->getExistingMethodRules());
    ...
    public function getLinkRuleIds(Mage_Core_Model_Abstract $object)
    {
        $select = $this
            ...
            ->select()
            ->from(
                $this->getTable('easyshippingrules/existing_method_rule'),
                array('easyshippingrules_rule_id')
            )
            ->where('easyshippingrules_existing_method_id = ?', $object->getId());

        return $this->_getReadAdapter()->fetchCol($select);
    }

    /**
     * Add link stores for existing method
     *
     * @param Mage_Core_Model_Abstract $object
     *
     * @return MatheusGontijo_EasyShippingRules_Model_Resource_Existingmethod
    ...
    protected function _addLinkStores(Mage_Core_Model_Abstract $object)
    {
        ...
        $table  = $this->getTable('easyshippingrules/existing_method_store');
        ...
        if ($delete) {
            $where = array(
                'easyshippingrules_existing_method_id = ?' => $object->getId(),
                'store_id IN (?)'                          => $delete,
            );
    ...
    /**
     * Add link rules for existing method
     *
     * @param Mage_Core_Model_Abstract $object
     * @param array                    $rules
     *
    ...
    public function lookupStoreIds($methodId)
    {
        ...
        $select = $adapter
            ->select()
            ->from(
                $this->getTable('easyshippingrules/existing_method_store'),
                array('store_id')
            )
            ->where('easyshippingrules_existing_method_id = ?', $methodId);
